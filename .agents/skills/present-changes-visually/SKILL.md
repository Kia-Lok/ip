...
3. Write to
   `.agents/skills/present-changes-visually/_temp/visual-diff.html` unless the
   user supplies an output path. This project-local directory is ignored by the
   skill's bundled `.gitignore`.
4. Run the bundled generator from the repository root:
   ...
   python3 .agents/skills/present-changes-visually/scripts/generate-split-view-diff.py \
     . HEAD WORKTREE \
     .agents/skills/present-changes-visually/_temp/visual-diff.html
   ```
